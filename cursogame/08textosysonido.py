...
def crearFuente(size):
    ruta = os.path.join("cursogame\image", "Kenvector_future_thin.ttf")
    return pygame.font.Font(ruta, size)

#Cargar Sonido
def cargarSonido(sonido):
    ruta = os.path.join("cursogame\sonido", sonido)
    return pygame.mixer.Sound(ruta)
def iniciarMusica():
    ruta = os.path.join("cursogame\sonido", "space-ranger.mp3")
    pygame.mixer.music.load(ruta)
    pygame.mixer.music.set_volume(0.6)
    pygame.mixer.music.play(loops= -1)# de manera continua
...
#fuentes
FRONT_MARCADOR = crearFuente(30)
FRONT_GAMEOVER = crearFuente(100)
FRONT_TITULO = crearFuente(80)
COLOR_MARCADOR = (255, 255, 255)
COLOR_GAMEOVER = (255, 0, 0)
COLOR_TITULO = ( 0, 255, 0)
...
class Marcador(pygame.sprite.Sprite):
    ...
    def actualizarTexto(self):
        self.image = FRONT_MARCADOR.render(str(self.puntos), False, COLOR_MARCADOR)
        self.rect = self.image.get_bounding_rect()
        self.rect.topright = (ANCHO -10, 10)
...
#MENSAJE GAME OVER
def gameover():
    gameover = FRONT_GAMEOVER.render("GAME OVER", False, COLOR_GAMEOVER)
    rect = gameover.get_bounding_rect()
    rect.center = (ANCHO // 2, ALTO // 2)
    PANTALLA.blit(gameover, rect)
...
def mostrarTitulo():
    ...
    titulo = FRONT_TITULO.render("SPACE FIGTH", False, COLOR_TITULO)
